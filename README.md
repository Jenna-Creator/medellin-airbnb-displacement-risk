...
The **Displacement Risk Index** is a difference of percentile ranks:

pct_homes_on_airbnb = 100 * listing_count / total_viviendas
displacement_risk_index = percentile_rank(pct_homes_on_airbnb) − percentile_rank(estrato)
...
The index also originally combined the two inputs using z-scores rather than percentile ranks — a second reviewer catch worth documenting, since it changed the results substantially. Z-scoring standardizes standard deviation, not range: `pct_homes_on_airbnb` stayed fat-tailed even after a log transform (one barrio's saturation exceeded 100% of its counted housing stock, versus a typical barrio in the low single digits), so its z-scores stretched out far further than a bounded 1–6 estrato scale ever could. That let a single extreme saturation value dominate the index regardless of estrato — exactly backwards from the point of the index, since the whole idea is to screen *out* high-saturation, high-estrato barrios (overtourism, not displacement risk) rather than let them top the list. Percentile ranks fix this at the root: they're bounded to [0, 1] no matter how skewed the underlying values are, so an outlier can only push its own rank to the top, not to an arbitrarily large number that overwhelms the other side of the subtraction.

A few design choices, and why:

- **Log-transforming saturation** is no longer needed for the index itself (ranks don't care about the underlying scale), but the log-transformed column is still used for the standalone housing-saturation map's color scale, for the same reason as before: a handful of barrios have Airbnb activity that dwarfs the rest, so a log scale keeps a few extreme outliers from flattening the rest of the map.
- **Percentile-ranking both variables** puts saturation and estrato on a common, bounded scale before comparing them, and is deliberately more robust to skew than z-scoring — see above.
- **Subtracting** the two ranks captures *mismatch*, not raw volume. A high score means unusually high saturation paired with unusually low estrato — the specific combination associated with displacement pressure. A wealthy, high-saturation neighborhood and a poor, low-saturation one both land near the middle of the range; only the poor-and-high-saturation combination scores high.
...
- **A minimum listing-count threshold was considered but not applied.** A diagnostic check (`17_check_listing_floor.py`) confirmed no top-ranked barrio in either city was riding an implausibly small number of listings to a high score once the housing-unit threshold was in place. (That check was run under the z-score version of the index; the percentile-rank version is structurally immune to this specific failure mode regardless, since no single value can dominate a bounded rank.)
...
**Medellín — Comuna 13 (San Javier) and Villa Hermosa top the index.** San Javier No. 2 and San Javier No. 1 lead, followed by five more San Javier barrios (Los Alcázares, Veinte de Julio, La Pradera, Belencito) and three from Villa Hermosa (San Miguel, Sucre, La Mansión) rounding out the top 10. San Javier is the city's lowest-estrato comuna and home to the Comuna 13 Graffitour, one of Medellín's most-visited tourist draws — this lines up with widely reported concerns about tourism-driven gentrification specifically in that comuna.

**El Poblado and Manila no longer dominate the index, and that's a fix, not a regression.** An earlier version of this index (using z-scores instead of percentile ranks — see "How the index was designed") let El Poblado's extreme saturation figure (Airbnb listings exceeded 100% of its 2018-counted housing stock) overwhelm the formula, pushing it to the top of the list despite being estrato 6. Percentile-ranking both inputs fixed that: a barrio's saturation can only rank as high as anyone else's, so high estrato now properly discounts it. El Poblado and Manila still show real, substantial Airbnb saturation — they're just correctly read as overtourism in an already-wealthy area, not displacement pressure on low-income residents, which is the distinction this index exists to draw.

**Barranquilla — Metropolitana, Suroccidente, and Norte Centro Histórico top the index, and Riomar's low-estrato pockets show up even though its wealthy core doesn't.** El Rosario and San Luis lead, with 20 de Julio, 7 de Abril, Los Girasoles (Metropolitana), El Golfo, La Paz, Ciudad Modesto (Suroccidente) — all estrato 1 — filling out most of the rest. Notably, San Salvador and Corregimiento La Playa also rank in the top 10 despite sitting inside Riomar, Barranquilla's wealthiest localidad overall — because those two specific barrios are themselves estrato 1–2, while Riomar's high-estrato core (El Poblado, Altamira, Villa del Este) shows real Airbnb saturation but doesn't rank as high-risk. That's the index discriminating correctly within a locality, not just between them.

**Barranquilla's ranking shifted less dramatically than Medellín's when the z-score bug was fixed**, since no Barranquilla barrio ever reached saturation levels extreme enough to break the old formula the way El Poblado did.
...
- **This index previously used z-scores instead of percentile ranks to combine saturation and estrato**, which let El Poblado's extreme saturation value dominate the composite score regardless of its high estrato — see "How the index was designed" for the full explanation. Fixed by switching both inputs to percentile ranks, which are bounded and can't be blown out by a single outlier the way z-scores can.
