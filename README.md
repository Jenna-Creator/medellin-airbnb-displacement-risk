...
Raw data → clean/dedupe → spatial join → estrato merge/index → interactive map. Each stage is its own script under `scripts/`, meant to be run in order:
...
| `05_choropleth.py` | Builds an interactive Folium/Leaflet map of the composite index, with hover tooltips per barrio. Outputs `data/processed/medellin_choropleth.html`. |
...
| `08_barranquilla_choropleth.py` | Same choropleth logic as script 05, for Barranquilla. Outputs `data/processed/barranquilla_choropleth.html`. |
...
`run_all.sh` reruns every script in `scripts/` in order (`01` through `13`), regenerating the full pipeline and all six maps in one command.
...
Python, pandas, geopandas, shapely, Folium. Data sourced via Apify (Airbnb scraper) and each city's official open-GIS ArcGIS Feature Services.
